...
const AccountPage = () => {
    ...
    const confirmEditPost = () => {
        const updatedPosts = user.posts.map(post => post.id === editingPost.id ? editingPost : post);
        ...
        <div className="account-container">
            ...
            {selectedPost && (
                <div className="modal-overlay" onClick={closeModal} style={{ backgroundColor: 'rgba(168, 85, 247, 0.3)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div className="expanded-post-modal" onClick={(e) => e.stopPropagation()} style={{ backgroundColor: 'white', padding: '20px', borderRadius: '10px', marginTop: '50px', width: '600px', maxWidth: '90%', boxSizing: 'border-box' }}>
                        <h2>Comments</h2>
                        ...
                        <div className="comment-box">
                            ...
                            {commentTab === 'view' ? (
                                <div className="comments-list">
                                    {selectedPost.comments?.length === 0 ? <p>No comments yet.</p> : selectedPost.comments.map(comment => (
                                        <div key={comment.id} className="comment"><strong style={{ color: '#a855f7' }}>{comment.poster}</strong>: <span style={{ color: 'black' }}>{comment.text}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
